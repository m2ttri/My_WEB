...
def create_album(request):
    ...
    return render(
        request, "album/create.html", {"form": form, "image_form": image_form}
    )
...
def album_detail(request, id):
    ...
    if request.method == "POST":
        add_image_form = MultipleImageForm(request.POST, request.FILES)
    ...
    return render(
        request, "album/detail.html", {"album": album, "add_image_form": add_image_form}
    )
...
def album_list(request):
    ...
    return render(request, "album/list.html", {"posts": posts})
...
def edit_album(request, id):
    album = get_object_or_404(Album, id=id)
    if request.method == "POST":
        form = AlbumEditForm(instance=album, data=request.POST)
        if form.is_valid():
            album = form.save(commit=False)
            album.author = request.user
            album.save()
    ...
    return render(request, "album/edit.html", {"form": form, "album": album})
